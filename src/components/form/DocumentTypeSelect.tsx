            ...
            <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {documentTypes.map((type) => (
                <Listbox.Option
                  key={type.id}
                  className={({ active }) =>
                    active ? 'bg-blue-50 text-blue-900 relative cursor-default select-none py-2 pl-10 pr-4' 
                ...
                >
                  {({ selected }) => (
                    <>
                      <span className={selected ? 'font-medium block truncate' : 'font-normal block truncate'}>
                        {type.name}
                      </span>
                      {selected ? (
